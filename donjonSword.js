...
export class DonjonSword extends Phaser.Scene{
    ...
    create(){
        ...
        // - DECORS DERRIERE
        const DonjonSword_Ground = carteDuNiveau.createLayer("DonjonSword_Ground",tileset);
        const DonjonSword_Back = carteDuNiveau.createLayer("DonjonSword_Back",tileset);
        ...
        this.player = this.physics.add.sprite(480, 3520, 'perso').setScale(1);
        ...
        const DonjonSword_Momies = carteDuNiveau.getObjectLayer('DonjonSword_Momies');
        DonjonSword_Momies.objects.forEach(eachMomie => {
        ...
        // - DECORS DEVANT
        const DonjonSword_Front = carteDuNiveau.createLayer("DonjonSword_Front",tileset);


        if (!this.playerState.getSword){
            this.sword = this.physics.add.image(480+16,2592+16,"sword").setScale(0.5);
        }

        this.physics.add.collider(this.shadow,this.sword,this.haveSword,null,this);

        // - COLLISIONS
        const DonjonSword_Collide = carteDuNiveau.createLayer("DonjonSword_Collide",tileset);
        DonjonSword_Collide.alpha = 0;
        DonjonSword_Collide.setCollisionByProperty({ collide: true }); 
        this.physics.add.collider(this.player, DonjonSword_Collide, this.collide, null,this);
        this.physics.add.collider(this.momies, DonjonSword_Collide);
    ...
    update(){
        ...
        if (this.canOut && (this.player.body.position.x >= 416 && this.player.body.position.x <= 544 && this.player.body.position.y >= 3584)){
            this.canOut = false;
			...
			this.time.delayedCall(500, () => {
					this.scene.start('MainCave', {entrance: "donjonSword", playerState : this.playerState});
			})
    ...
    haveSword(shadow,sword){
        this.playerState.getSword = true;
        this.add.image(600,462,"sword").setOrigin(0,0).setScrollFactor(0).setScale(0.75);
        this.sword.alpha = 0;
        this.sword.destroy();
        this.playerState.canMove = false;
        this.player.setVelocityX(0);
        this.player.setVelocityY(0);
        this.tutoText = this.add.text(750,450,'Click to attack !',{ fontSize:'40px',fill:'#ff4967', fontStyle:"bold"}).setScrollFactor(0);
    }
}