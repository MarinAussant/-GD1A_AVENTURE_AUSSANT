...
export class Caveau2 extends Phaser.Scene{
    ...
    create(){
        ...
        const Cave_Caveau2_Donjon_Back = carteDuNiveau.createLayer("Cave_Caveau2_Donjon_Back",tileset);

        this.coffres = this.physics.add.group({allowGravity: false,immovable : true});

        if(!this.playerState.getCoffre1Caveau2){
            this.coffre1Caveau2 = this.physics.add.sprite(320+32,1312+16,"coffreDevantCave");
            this.coffres.add(this.coffre1Caveau2);
        }
        if(!this.playerState.getCoffre2Caveau2){
            this.coffre2Caveau2 = this.physics.add.sprite(606+32,2240+16,"coffreDevantCave");
            this.coffres.add(this.coffre2Caveau2);
        }
        ...
        this.player = this.physics.add.sprite(500, 2065, 'perso').setScale(1);
        ...
        const Cave_Caveau2_Momies = carteDuNiveau.getObjectLayer('Cave_Caveau2_Momies');
        Cave_Caveau2_Momies.objects.forEach(eachMomie => {
        ...
        // - DECORS DEVANT
        const Cave_Caveau2_Donjon_Front = carteDuNiveau.createLayer("Cave_Caveau2_Donjon_Front",tileset);
        const Cave_Caveau2_Wall_Front = carteDuNiveau.createLayer("Cave_Caveau2_Wall_Front",tileset);
        const Cave_Caveau2_Exit = carteDuNiveau.createLayer("Cave_Caveau2_Exit",tileset);

        // - COLLISIONS
        const Cave_Caveau2_Collide = carteDuNiveau.createLayer("Cave_Caveau2_Collide",tileset);
        Cave_Caveau2_Collide.alpha = 0;
        Cave_Caveau2_Collide.setCollisionByProperty({ collide: true }); 
        this.physics.add.collider(this.player, Cave_Caveau2_Collide, this.collide, null,this);
        this.physics.add.collider(this.momies, Cave_Caveau2_Collide);
    ...
    update(){
        ...
        if (this.canOut && (this.player.body.position.x >= 532 && this.player.body.position.y >= 2016 && this.player.body.position.y <= 2109)){
            this.canOut = false;
			...
			this.time.delayedCall(500, () => {
					this.scene.start('Outdoor', {entrance: "caveau2", playerState : this.playerState});
			})
    ...
    lootCoffre(zone, coffre){
        ...
        coffre.alpha = 0;
        if(coffre == this.coffre1Caveau2){
            this.playerState.getCoffre1Caveau2 = true;
        }
        if(coffre == this.coffre2Caveau2){
            this.playerState.getCoffre2Caveau2 = true;
        }
