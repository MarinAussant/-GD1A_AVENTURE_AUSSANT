...
export class Secret1 extends Phaser.Scene{
    ...
    create(){
        ...
        const Cave_Secret1_Donjon_Back = carteDuNiveau.createLayer("Cave_Secret1_Donjon_Back",tileset);

        this.coffres = this.physics.add.group({allowGravity: false,immovable : true});

        if(!this.playerState.getCoffreSecret1){
            this.coffreSecret1 = this.physics.add.sprite(1312+32,3008+16,"coffreDevantCave");
            this.coffres.add(this.coffreSecret1);
        }
        ...
        this.player = this.physics.add.sprite(1500, 3260, 'perso').setScale(1);
        ...
        // - DECORS DEVANT
        const Cave_Secret1_Donjon_Front = carteDuNiveau.createLayer("Cave_Secret1_Donjon_Front",tileset);
        const Cave_Secret1_Wall_Front = carteDuNiveau.createLayer("Cave_Secret1_Wall_Front",tileset);
        const Cave_Secret1_Exit = carteDuNiveau.createLayer("Cave_Secret1_Exit",tileset);

        // - COLLISIONS
        const Cave_Secret1_Collide = carteDuNiveau.createLayer("Cave_Secret1_Collide",tileset);
        Cave_Secret1_Collide.alpha = 0;
        Cave_Secret1_Collide.setCollisionByProperty({ collide: true }); 
        this.physics.add.collider(this.player, Cave_Secret1_Collide, this.collide, null,this);
    ...
    update(){
        ...
        if (this.canOut && (this.player.body.position.x >= 1532 && this.player.body.position.y >= 3232 && this.player.body.position.y <= 3293)){
            this.canOut = false;
			...
			this.time.delayedCall(500, () => {
					this.scene.start('Outdoor', {entrance: "secret1", playerState : this.playerState});
			})
    ...
    lootCoffre(zone, coffre){
        ...
        coffre.alpha = 0;
        if(coffre == this.coffreSecret1){
            this.playerState.getCoffreSecret1 = true;
        }
