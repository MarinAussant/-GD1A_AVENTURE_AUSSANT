...
export class Secret3 extends Phaser.Scene{
    ...
    create(){
        ...
        const Cave_Secret3_Donjon_Back = carteDuNiveau.createLayer("Cave_Secret3_Donjon_Back",tileset);

        this.coffres = this.physics.add.group({allowGravity: false,immovable : true});

        if(!this.playerState.getCoffreSecret3){
            this.coffreSecret3 = this.physics.add.sprite(2176+32,4320+16,"coffreDevantCave");
            this.coffres.add(this.coffreSecret3);
        }
        ...
        // - DECORS DEVANT
        const Cave_Secret3_Donjon_Front = carteDuNiveau.createLayer("Cave_Secret3_Donjon_Front",tileset);
        ...
        this.physics.add.collider(this.player, Cave_Secret3_Collide, this.collide, null,this);
    ...
    update(){
        ...
        if (this.canOut && (this.player.body.position.x <= 1969 && this.player.body.position.x >= 1888 && this.player.body.position.y >= 4450 )){
            this.canOut = false;
    ...
    lootCoffre(zone, coffre){
        ...
        coffre.alpha = 0;
        if(coffre == this.coffreSecret3){
            this.playerState.getCoffreSecret3 = true;
        }
